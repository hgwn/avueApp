...
- (void)viewDidLoad {
    ...
    //创建一个高20的假状态栏
    UIView *statusBarView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, self.view.bounds.size.width, 44)];
    //设置成绿色
...
-(WKWebView *)webView
{
    if (is_iPhoneXSerious) {
        ...
        if (_webView == nil) {
                if (self.navigationController.navigationBar.hidden) {
                     _webView = [[WKWebView alloc] initWithFrame:CGRectMake( 0, 0, WKSCREEN_WIDTH, WKSCREEN_HEIGHT - WKSTATUS_BAR_HEIGHT - WKNAVIGATION_BAR_HEIGHT-64) configuration:self.config];
                }else{
        //            _webView = [[WKWebView alloc] initWithFrame:CGRectMake( 0, 0, WKSCREEN_WIDTH, WKSCREEN_HEIGHT - WKSTATUS_BAR_HEIGHT - WKNAVIGATION_BAR_HEIGHT - APPSTATUS_BAR_HEIGHT) configuration:self.config];
                    _webView = [[WKWebView alloc] initWithFrame:CGRectMake( 0, 30, WKSCREEN_WIDTH, WKSCREEN_HEIGHT-64) configuration:self.config];
    ...
    else
    {
        ...
        //            _webView = [[WKWebView alloc] initWithFrame:CGRectMake( 0, 0, WKSCREEN_WIDTH, WKSCREEN_HEIGHT - WKSTATUS_BAR_HEIGHT - WKNAVIGATION_BAR_HEIGHT - APPSTATUS_BAR_HEIGHT) configuration:self.config];
                    _webView = [[WKWebView alloc] initWithFrame:CGRectMake( 0, 20, WKSCREEN_WIDTH, WKSCREEN_HEIGHT-20) configuration:self.config];
 ...
 *  @param htmlName html页面文件名称
 */
- (void)loadLocalHTML:(nonnull NSString *)newHtml:(NSURL *)baseURL {

    [self.webView loadHTMLString:newHtml baseURL:baseURL];
...
- (void)webView:(WKWebView *)webView didReceiveAuthenticationChallenge:(NSURLAuthenticationChallenge *)challenge completionHandler:(void (^)(NSURLSessionAuthChallengeDisposition disposition, NSURLCredential * _Nullable credential))completionHandler{
    ...
    if ([authenticationMethod isEqualToString:NSURLAuthenticationMethodDefault]
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            [alertController setModalPresentationStyle:UIModalPresentationFullScreen];
